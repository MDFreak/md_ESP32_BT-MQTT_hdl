...
// ----------------------------------------------------------------
  ...
  // --- Bluetooth
    #if (USE_BTOOTH > OFF)
        void*    pbtooth_dev_data = NULL;
        uint16_t btooth_FIELD_IDX_MAX = 0;
      // --- Bluetti
        #if (USE_BLUETTI)
            ...
            // --- MQTT value as string
              static String valBLU[BLUETTI_FIELD_IDX_MAX];
                /*
                  // INFO device offset sort
                ...
                */
            // --- MQTT publish flag
              static int8_t pubBLU[BLUETTI_FIELD_IDX_MAX];
                /*
                  // INFO device offset sort
                ...
                */
              // MQTT topic
              static String topBLU[BLUETTI_FIELD_IDX_MAX];
                /*
                  // INFO device offset sort
  ...
  void setup()
    {
      ...
      // --- network
        ...
        // start bluetooth
          #if (USE_BTOOTH)
              #if (USE_BLUETTI)
                  btooth_FIELD_IDX_MAX += BLUETTI_FIELD_IDX_MAX;
                  SHEXVAL(" init bluetooth...", (uint32_t) pbtooth_dev_state);
                  pbtooth_dev_state = getpDevField();
                  initBluetooth();
                  SHEXVAL(" ...init bluetooth", (uint32_t) pbtooth_dev_state);
                #endif
              #if (USE_CONRAD_UIP)
                        ...
                        if ( ((device_field_data_t*) pbtooth_dev_state)[outpIdx].f_new > OFF) // new value
                          {
                            ...
                            pbtooth_dev_state[outpIdx].f_new = FALSE;
                              ...
                              switch (i)
                                {
                                  // INFO device offset sort
                                    case DEVICE_TYPE:
                                        pbtooth_dev_state[i].p_f_value = (void*) &BLUDEVICE_TYPE;
                                      break;
                                    case SERIAL_NUMBER:
                                          pbtooth_dev_state[i].p_f_value = (void*) &BLUSERIAL_NUMBER;
                                      break;
                                    case  ARM_VERSION:
                                          pbtooth_dev_state[i].p_f_value = (void*) &BLUARM_VERSION;
                                      break;
                                    case  DSP_VERSION:
                                          pbtooth_dev_state[i].p_f_value = (void*) &BLUDSP_VERSION;
                                      break;
                                    case  DC_INPUT_POWER:
                                          pbtooth_dev_state[DC_INPUT_POWER].p_f_value = (void*) &BLUDC_INPUT_POWER;
                                      break;
                                    case  AC_INPUT_POWER:
                                          pbtooth_dev_state[AC_INPUT_POWER].p_f_value = (void*) &BLUAC_INPUT_POWER;
                                      break;
                                    case  AC_OUTPUT_POWER:
                                          pbtooth_dev_state[AC_OUTPUT_POWER].p_f_value = (void*) &BLUAC_OUTPUT_POWER;
                                      break;
                                    case  DC_OUTPUT_POWER:
                                          pbtooth_dev_state[DC_OUTPUT_POWER].p_f_value = (void*) &BLUDC_OUTPUT_POWER;
                                      break;
                                    case  POWER_GENERATION:
                                          pbtooth_dev_state[POWER_GENERATION].p_f_value = (void*) &BLUPOWER_GENERATION;
                                      break;
                                    case  TOTAL_BATT_PERC:
                                          pbtooth_dev_state[TOTAL_BATT_PERC].p_f_value = (void*) &BLUTOTAL_BATT_PERC;
                                      break;
                                    case  AC_OUTPUT_ON:
                                          pbtooth_dev_state[AC_OUTPUT_ON].p_f_value = (void*) &BLUARM_VERSION;
                                      break;
                                    case  DC_OUTPUT_ON:
                                          pbtooth_dev_state[DC_OUTPUT_ON].p_f_value = (void*) &BLUDC_OUTPUT_ON;
                                      break;
                                  // INFO internal
                                    case  AC_OUTPUT_MODE:
                                          pbtooth_dev_state[AC_OUTPUT_MODE].p_f_value = (void*) &BLUAC_OUTPUT_MODE;
                                      break;
                                    case  INTERN_AC_VOLT:
                                          pbtooth_dev_state[INTERN_AC_VOLT].p_f_value = (void*) &BLUINTERN_AC_VOLT;
                                      break;
                                    case  INTERN_CURR_1:
                                          pbtooth_dev_state[INTERN_CURR_1].p_f_value = (void*) &BLUINTERN_CURR_1;
                                      break;
                                    case  INTERN_POWER_1:
                                          pbtooth_dev_state[INTERN_POWER_1].p_f_value = (void*) &BLUINTERN_POWER_1;
                                      break;
                                    case  INTERN_AC_FREQ:
                                          pbtooth_dev_state[INTERN_AC_FREQ].p_f_value = (void*) &BLUINTERN_AC_FREQ;
                                      break;
                                    case  INTERN_CURR_2:
                                          pbtooth_dev_state[INTERN_CURR_2].p_f_value = (void*) &BLUINTERN_CURR_2;
                                      break;
                                    case  INTERN_POWER_2:
                                          pbtooth_dev_state[INTERN_POWER_2].p_f_value = (void*) &BLUINTERN_POWER_2;
                                      break;
                                    case  AC_INPUT_VOLT:
                                          pbtooth_dev_state[AC_INPUT_VOLT].p_f_value = (void*) &BLUAC_INPUT_VOLT;
                                      break;
                                    case  INTERN_CURR_3:
                                          pbtooth_dev_state[INTERN_CURR_3].p_f_value = (void*) &BLUINTERN_CURR_3;
                                      break;
                                    case  INTERN_POWER_3:
                                          pbtooth_dev_state[INTERN_POWER_3].p_f_value = (void*) &BLUINTERN_POWER_3;
                                      break;
                                    case  AC_INPUT_FREQ:
                                          pbtooth_dev_state[AC_INPUT_FREQ].p_f_value = (void*) &BLUAC_INPUT_FREQ;
                                      break;
                                    case  INT_DC_INP_VOLT:
                                          pbtooth_dev_state[INT_DC_INP_VOLT].p_f_value = (void*) &BLUINT_DC_INP_VOLT;
                                      break;
                                    case  INT_DC_INP_POW:
                                          pbtooth_dev_state[INT_DC_INP_POW].p_f_value = (void*) &BLUINT_DC_INP_POW;
                                      break;
                                    case  INT_DC_INP_CURR:
                                          pbtooth_dev_state[INT_DC_INP_CURR].p_f_value = (void*) &BLUINT_DC_INP_CURR;
                                      break;
                                    case  PACK_NUM_MAX:
                                          pbtooth_dev_state[PACK_NUM_MAX].p_f_value = (void*) &BLUPACK_NUM_MAX;
                                      break;
                                    case  TOTAL_BATT_VOLT:
                                          pbtooth_dev_state[TOTAL_BATT_VOLT].p_f_value = (void*) &BLUTOTAL_BATT_VOLT;
                                      break;
                                    case  TOTAL_BATT_CURR:
                                          pbtooth_dev_state[TOTAL_BATT_CURR].p_f_value = (void*) &BLUTOTAL_BATT_CURR;
                                      break;
                                    case  PACK_NUM:
                                          pbtooth_dev_state[PACK_NUM].p_f_value = (void*) &BLUPACK_NUM;
                                      break;
                                    case  PACK_STATUS:
                                          pbtooth_dev_state[PACK_STATUS].p_f_value = (void*) &BLUPACK_STATUS;
                                      break;
                                    case  PACK_VOLTAGE:
                                          pbtooth_dev_state[PACK_VOLTAGE].p_f_value = (void*) &BLUPACK_VOLTAGE;
                                      break;
                                    case  PACK_BATT_PERC:
                                          pbtooth_dev_state[PACK_BATT_PERC].p_f_value = (void*) &BLUPACK_BATT_PERC;
                                      break;
                                    //case  CELL_VOTAGES:
                                      //break;
                                  // CONTROL elements
                                    case  UPS_MODE:
                                          pbtooth_dev_state[UPS_MODE].p_f_value = (void*) &BLUUPS_MODE;
                                      break;
                                    case  SPLIT_PHASE_ON:
                                          pbtooth_dev_state[SPLIT_PHASE_ON].p_f_value = (void*) &BLUSPLIT_PHASE_ON;
                                      break;
                                    case  SPLIT_PH_MACH_MODE:
                                          pbtooth_dev_state[SPLIT_PH_MACH_MODE].p_f_value = (void*) &BLUSPLIT_PH_MACH_MODE;
                                      break;
                                    case  SET_PACK_NUM:
                                          pbtooth_dev_state[SET_PACK_NUM].p_f_value = (void*) &BLUSET_PACK_NUM;
                                      break;
                                    case  SET_AC_OUT_ON:
                                          pbtooth_dev_state[SET_AC_OUT_ON].p_f_value = (void*) &BLUSET_AC_OUT_ON;
                                      break;
                                    case  SET_DC_OUT_ON:
                                          pbtooth_dev_state[SET_DC_OUT_ON].p_f_value = (void*) &BLUSET_DC_OUT_ON;
                                      break;
                                    case  GRID_CHANGE_ON:
                                          pbtooth_dev_state[GRID_CHANGE_ON].p_f_value = (void*) &BLUGRID_CHANGE_ON;
                                      break;
                                    case  TIME_CTRL_ON:
                                          pbtooth_dev_state[TIME_CTRL_ON].p_f_value = (void*) &BLUTIME_CTRL_ON;
                                      break;
                                    case  BATT_RANGE_START:
                                          pbtooth_dev_state[BATT_RANGE_START].p_f_value = (void*) &BLUBATT_RANGE_START;
                                      break;
                                    case  BATT_RANGE_END:
                                          pbtooth_dev_state[BATT_RANGE_END].p_f_value = (void*) &BLUBATT_RANGE_END;
                                      break;
                                    case  BLUETOOTH_CONN:
                                          pbtooth_dev_state[BLUETOOTH_CONN].p_f_value = (void*) &BLUBLUETOOTH_CONN;
                                      break;
                                    case  AUTO_SLEEP_MODE:
                                          pbtooth_dev_state[AUTO_SLEEP_MODE].p_f_value = (void*) &BLUAUTO_SLEEP_MODE;
                                      break;
                                    case  LED_CONTROL:
                                          pbtooth_dev_state[LED_CONTROL].p_f_value = (void*) &BLULED_CONTROL;
                                      break;
                                }
                              //S2VAL(" init ", DEVICE_F_NAMES[i], (uint32_t) pbtooth_dev_state[i].p_f_value);
                              topBLU[i] = topbluDevice + DEVICE_F_NAMES[i];
              ...
              if (errMQTT == MD_OK) // connected
                {
                  ...
                  while (anzMQTTmsg > 0)
                    {
                      //ptopic = pMQTTRd->topic + strlen(MQTT_BLUETTI_TOPDEV); // remove device ID
