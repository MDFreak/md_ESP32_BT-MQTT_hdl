  ...
  void eeprom_read()
    {
      /*
        ...
        Serial.print("  mqtt_password ");
            Serial.println(F(wifiConfig.mqtt_password));
        Serial.print("  bluetti_device_id ");
            Serial.println(F(wifiConfig.bluetti_device_id));
        Serial.print("  mqtt_port ");
            Serial.println(F(wifiConfig.mqtt_port));
        Serial.print("  bluetti_device_id ");
            Serial.println(F(wifiConfig.mqtt_server));
        Serial.print("  mqtt_username ");
            Serial.println(F(wifiConfig.mqtt_username));
        Serial.print("  ota_password ");
            Serial.println(F(wifiConfig.ota_password));
        Serial.print("  ota_username ");
            Serial.println(F(wifiConfig.ota_username));
        //Serial.print("  salt ");
        //    Serial.println(F(wifiConfig.salt));
          EEPROM.begin(512);
  ...
  void initBWifi(bool resetWifi)
    {
      ...
      /*
      WiFiManagerParameter custom_mqtt_server   ("server", "10.0.0.230", mqtt_server, 40);
      WiFiManagerParameter custom_mqtt_port     ("port", "1883", mqtt_port, 6);
      WiFiManagerParameter custom_mqtt_username ("username", "", "", 40);
      WiFiManagerParameter custom_mqtt_password ("password", "", "", 40, "type=password");
      WiFiManagerParameter custom_ota_username  ("ota_username", "", "", 40);
      WiFiManagerParameter custom_ota_password  ("ota_password", "", "", 40, "type=password");
      WiFiManagerParameter custom_bluetti_device("bluetti", "AC3002235000574654", bluetti_device_id, 40);
  ...
  String processorWebsiteUpdates(const String& var)
    {
      /*
        if(var == "IP")  { return String(WiFi.localIP().toString()); }
        if(var == "RSSI"){ return String(WiFi.RSSI()); }
        if(var == "SSID"){ return String(WiFi.SSID()); }
        if(var == "MAC") { return String(WiFi.macAddress()); }
        if     (var == "RUNTIME")
          {
